...
- **Day-loss enforcement is via auto-disarm, not a per-submit gate branch.** The multi-broker spec lists "max total day loss" as the final global gate layer whose breach = master disarm. Because command handling and account-refresh reconcile are serialized through the one writer loop, checking day-loss on every account update and flipping the master switch is equivalent to (and the spec's stated effect of) a per-submit branch — a submit after a breach is blocked by the master-armed check. `BreachedDayLoss` is a pure helper the coordinator calls on account refresh.

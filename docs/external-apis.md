...
- **Alpaca:** paper/live REST and trade-update WebSocket execution, plus
  ...
  up-to-50-day REL VOL baselines when configured; IEX and unavailable clients
...
- OpenD 3203 `SnapshotBasicData.volumeRatio` remains an optional provider field at the feed boundary; the Scanner does not consume or expose it. Scanner `Vol` and dollar turnover use the snapshot's base daily fields; `Session Volume` uses the matching nullable `PreMarket`, base RTH, `AfterMarket`, or `Overnight` volume field and never adds session components. REL VOL is the snapshot daily volume divided by the mean raw Alpaca SIP daily volume from up to 50 preceding completed trading dates.
...
- OpenD rank responses 3410/3413/3411/3412 remain Scanner discovery sources for price, change, and an initial matching-session volume. Snapshot 3203 supplies the base daily volume/dollar turnover and matching-session volume used for rows and filters; phase/pool-day provenance prevents stale session fallback. The UI thresholds remain raw shares/USD, while daily and Session Volume display units are saved independently.
...
- Historical chart/backfill requests use completed offline-NYSE-calendar horizons and persisted explored-range coverage. Scanner REL VOL is separate: for sticky-pool symbols it requests one bounded Alpaca SIP raw `1Day` range covering up to the prior 50 trading dates, excluding the represented snapshot date. Raw Scanner bars are not persisted; only the compact in-memory mean/sample count survives until the represented-date cache expires.

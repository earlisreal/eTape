...
- OpenD 3203 `SnapshotBasicData.volumeRatio` is an optional provider Volume Ratio multiplier. RTH Top Movers may provide the same optional field as a fallback; pre-market, after-hours, and overnight rank endpoints do not. eTape preserves it as unavailable when absent or invalid and does not invent an extended-hours formula.

...
- OpenD 3203 `SnapshotBasicData.volumeRatio` remains an optional provider field at the feed boundary; the Scanner does not consume or expose it. Scanner volume and dollar turnover use the snapshot's base daily fields; pre-market, after-hours, and overnight components are not added. REL VOL is the snapshot daily volume divided by the mean raw Alpaca SIP daily volume from up to 50 preceding completed trading dates.
...
- OpenD rank responses 3410/3413/3411/3412 remain Scanner discovery sources for price and change. Snapshot 3203 supplies the base daily volume and dollar turnover used for rows and filters; rank/session totals never substitute for missing daily fields. The UI thresholds remain raw shares/USD and saved values are unchanged.

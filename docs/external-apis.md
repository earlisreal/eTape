...
- OpenD rank responses 3410/3413/3411/3412 and snapshot 3203 provide the Scanner's Dollar Turnover. eTape selects the matching pre-market, RTH, after-hours, or overnight field, rejects absent/negative/non-finite values, prefers a valid snapshot over rank data, and never derives turnover from price times volume. The UI threshold is stored as raw USD and only filters new board admissions; rows and values are scoped to the current ET session instance.

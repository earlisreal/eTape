...
Native TCP framing and protobuf client. Handles handshake, serial-number correlation, keepalive, subscriptions, snapshots, K-lines, scanner/news, and trade messages. Ticker updates preserve OpenD's raw `TickerType`, `typeSign`, and `PushDataType`, while the market-data core applies the centralized Trade-Report Condition eligibility table; known unusual/derived reports retain their evidence and may be volume-only or conditionally price-forming, and unrecognized values fail closed as unknown. Cached ticker seeds are sorted oldest-first and capped at 1,000 before they share the live event path. Security-snapshot requests (protocol 3203) register their pending response first, then pass through one injected-clock gate immediately before the serialized wire send, with a 550 ms minimum spacing; cancellation does not reserve a future slot, and the gate persists across reconnects without holding socket or order locks. This gate is shared by callers using this Client instance only. The reader counts/rate-limits raw push-buffer overflow warnings; established disconnects and successful reconnect resyncs are logged. Default downstream: `127.0.0.1:11111`. `pb/` is generated; never hand-edit. Preserve reconnect resync. Test: `go test ./internal/feed/opend`.

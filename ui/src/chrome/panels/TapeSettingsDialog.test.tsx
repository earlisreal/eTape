...
describe("TapeSettingsDialog", () => {
  it("identifies the selected symbol while status is still warming", () => {
    render(<TapeSettingsDialog chrome={chrome} minSize={0} hoverEnabled={false} symbol="US.NVDA" onClose={() => {}} onApply={() => {}} />);
    expect(screen.getByText("US.NVDA")).toBeTruthy();
  });

  it("shows the current minimum trade size", () => {
    render(<TapeSettingsDialog chrome={chrome} minSize={250} hoverEnabled={false} symbol="US.AAPL" onClose={() => {}} onApply={() => {}} />);
    expect((screen.getByLabelText("minimum trade size") as HTMLInputElement).value).toBe("250");
  });

  it("applies an edited value on Ok", () => {
    const onApply = vi.fn();
    render(<TapeSettingsDialog chrome={chrome} minSize={0} hoverEnabled={false} symbol="US.AAPL" onClose={() => {}} onApply={onApply} />);
    fireEvent.change(screen.getByLabelText("minimum trade size"), { target: { value: "500" } });
    fireEvent.click(screen.getByRole("button", { name: "Ok" }));
    expect(onApply).toHaveBeenCalledWith(500, false);
  });

  it("Defaults resets the draft to 0", () => {
    const onApply = vi.fn();
    render(<TapeSettingsDialog chrome={chrome} minSize={300} hoverEnabled={true} symbol="US.AAPL" onClose={() => {}} onApply={onApply} />);
    ...
    fireEvent.click(screen.getByRole("button", { name: "Ok" }));
    expect(onApply).toHaveBeenCalledWith(0, false);
  });
  ...
  it("closes without applying on Cancel", () => {
    const onApply = vi.fn();
    const onClose = vi.fn();
    render(<TapeSettingsDialog chrome={chrome} minSize={0} hoverEnabled={false} symbol="US.AAPL" onClose={onClose} onApply={onApply} />);
  ...
  it("keeps hover details off by default and applies an enabled choice", () => {
    const onApply = vi.fn();
    render(<TapeSettingsDialog chrome={chrome} minSize={0} hoverEnabled={false} symbol="US.AAPL" onClose={() => {}} onApply={onApply} />);
    ...
    fireEvent.click(screen.getByRole("button", { name: "Ok" }));
    expect(onApply).toHaveBeenCalledWith(0, true);
  });
});

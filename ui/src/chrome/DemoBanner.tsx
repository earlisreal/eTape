...
// Persistent, unmissable strip shown whenever the engine reports demo mode
// (sys.session). Safety-critical: practice/demo trading must never be
...
// Structural clone of ReplayBanner.tsx — same subscription/restart-detection
// shape — but demo and replay are mutually exclusive session modes, each
// banner independently gates on its own `s.mode`, so exactly one (or
// neither) renders at a time. Visual accent (palette.demo, a magenta/plum
// distinct from ReplayBanner's amber palette.warn) is chosen so REPLAY and
// DEMO are never confusable with each other at a glance — see palette.ts.
//
...
export function DemoBanner({ session, engineState, onGoLive }: {
  ...
  // banner while unconfirmed; flashing DEMO on the common live-reload case

...
export function ReconnectOverlay({ state, children }: { state: ConnState; children: ReactNode }): JSX.Element {
  ...
  return (
    <div style={{ position: "relative", height: "100%" }}>
      <div style={{ height: "100%", opacity: isStopped || showOverlay ? 0.4 : 1, transition: "opacity 120ms" }}>
        {children}
      </div>
      {(isStopped || showOverlay) && (
        <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center",
          background: palette.bg, color: palette.textMuted, pointerEvents: "none" }}>
          {isStopped ? "Engine stopped" : state === "connecting" ? "connecting…" : "reconnecting…"}
        </div>
      )}
    </div>
  );

...
const r = (symbol: string, changePct: number) =>
  ({ symbol, shortSellRestricted: false, changePct, last: 1, floatShares: 1_000_000, volume: 1000, turnover: null, relativeVolume: null, shortInterest: null, shortInterestAsOf: null });
...
describe("ScannerStore", () => {
  it("normalizes legacy rows without REL VOL", () => {
    ...
    expect(s.view("premarket").rows[0].turnover).toBeNull();
...
const rankMsg = (kind: "snapshot" | "delta", symbols: string[]) => ({
  ...
  payload: { refreshedAt: "2026-07-06T13:00:00Z", rows: symbols.map((symbol) => ({ symbol, shortSellRestricted: false, changePct: 5, last: 1, floatShares: 1, volume: 1, turnover: null, relativeVolume: null, shortInterest: null, shortInterestAsOf: null })) },
});
